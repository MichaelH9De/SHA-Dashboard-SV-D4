...
export default function ValueByProjectChart() {
  const items = seedProjects
    .map((project) => {
      const value = getProjectTotals(project).achievedValue
      return { label: project.name, value, valueLabel: formatCurrency(value) }
    })
  ...
  return (
    <Card>
      <h3 className="text-lg font-semibold text-slate-950">Value by project</h3>
